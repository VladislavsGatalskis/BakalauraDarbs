...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, annotation_json, images_dir, dataset_type="train"):
        ...
        # Split the dataset, if train, get 80%, else 20%
        training_set_amount = 0.80 # 80% is training set
        len_images = len(coco_json['images'])
        if dataset_type == "train":
            img_range = [0, int(len_images * training_set_amount)]
        else:
            img_range = [int(len_images * training_set_amount), len_images]
...
def load_test_model(num_classes, model_path=None):
    ...
    model.keras_model.summary(line_length=200) # Show model architecture
...
# Selecting Images By ID
def test_image_by_id(test_model, dataset_test, inference_config, image_id):
    original_image, image_meta, gt_class_id, gt_bbox, gt_mask = \
